...
evaluate_partition_semisup <-
  ...
  cox.part <- survival::coxph(survival::Surv(time, status) ~  Cluster, data.cc)
  cox.latent <- survival::coxph(survival::Surv(time, status) ~  Latent, data.cc)
  cox.sup <- survival::coxph(survival::Surv(time, status) ~  Sup, data.cc)
  ...
  cindLat.only <-
    Hmisc::rcorrp.cens(data.cc$LP, data.cc$LP.latent,
                       survival::Surv(data.cc$time, data.cc$status), method = 2)
  cindSup.only <-
