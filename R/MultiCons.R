...
#' @param Clustering_selection If DB is data, clustering algorithm to select among. Must be included in default value.
...
#' @examples
#' # TO DO
MultiCons <- function(DB, Clust_entry = F,
                      Clustering_selection = c("kmeans", "pam", "OPTICS", "agghc",
                                               "AGNES", "DIANA", "MCLUST", "CMeans",
                                               "FANNY", "BaggedClust"),
                      num_algo = 10, maxClust = 10, sim.indice = "jaccard", returnAll = F,
                      Plot = T, verbose = F){

  if(Plot) {
    requireNamespace("igraph", quietly = F)
    requireNamespace("RColorBrewer", quietly = F)
    requireNamespace("graphics", quietly = F)
  }

  #  1 Generate multiple base clusterings of the dataset;
  if (!Clust_entry){
    if("OPTICS" %in% Clustering_selection) {requireNamespace("dbscan", quietly = F)}
    if("MCLUST" %in% Clustering_selection) {requireNamespace("mclust", quietly = F)}
    if (length(intersect(c("pam", "AGNES", "DIANA", "FANNY"), Clustering_selection)) > 0) {
      requireNamespace("cluster", quietly = F)
    }
    if (length(intersect(c("CMeans", "BaggedClust"), Clustering_selection)) > 0) {
      requireNamespace("e1071", quietly = F)
    }

    lesBaseClust <- sample(Clustering_selection, num_algo, replace = T)
    ...
    Clust <- lapply(1:num_algo, function(i) {
      ...
      switch(lesBaseClust[i],
             ...
             OPTICS = dbscan::extractDBSCAN(dbscan::optics(DB, sample(1:10, 1)), eps_cl =
                                              stats::runif(1, 0.3, 1))$cluster,
             agghc = stats::cutree(stats::hclust(stats::dist(DB),
             ...
             MCLUST = mclust::Mclust(DB, G = 2:maxClust, verbose = F,
                                     modelNames = sample(
  ...
  if (Plot) {
    Mat <- do.call(rbind, lapply(1:(L - 1), function(i){
      vers <- sapply(ConsVctrs[[i]], function(clust){
        names(ConsVctrs[[i + 1]])[which(sapply(ConsVctrs[[i + 1]], function(a){
          any(clust %in% a)
        }))]
      })
    ...
    df <- data.frame(Mat)
    df_graph <- igraph::graph_from_data_frame(df, directed = F)
    palettes <-
      RColorBrewer::brewer.pal(n = min(9, L), name = "Set3")[1:L - 9 * (1:L %/% 9)]
    ts <- unlist(taille)
    ...
    p <- igraph::plot.igraph(
      df_graph,
      layout = igraph::layout_as_tree(df_graph,
